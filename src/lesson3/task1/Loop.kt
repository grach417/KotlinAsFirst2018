 ...
 */
fun sinCos(eps: Double, u: Double, f: Int, w: Double, b: Int, s: Double): Double {
    var n = f
    var g = s
    var v = b
    var k = w
    while (abs(pow(u, k) / factorial(n)) >= eps) {
        g += v * pow(u, k) / factorial(n)
        v *= -1
        k += 2
        n += 2
    }
